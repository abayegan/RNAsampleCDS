		...
		for ch1 in NUCL:
			for ch2 in NUCL:
				for x in range(0,3*(n-k)+8):
					ZB[(k,x,ch1,ch2)] = 0.0
				ZB[(n,0,ch1,ch2)] = 1.0 #Necessary for correct computation of PSSM and codon usage
	if constraint==None:
		...
		for ch3 in NUCL:
			for ch4 in NUCL:
				for x in range(0,3*k+8): #Warning: 3k+2 doesn't work.
					ZF[(k,x,ch3,ch4)] = 0.0
				ZF[(0,0,ch3,ch4)] = 1.0 #necessary for correct computation of pssm and codonUsage index
	if constraint==None:
