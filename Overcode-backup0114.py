...
import sys,os,copy,random,math,shlex
...
# PSSM is a dictionary s.t PSSM[(pos,ch)]= frequency of nucleotide 'ch' at position 'pos' of the sequence.
	...
	fout.close()
	cmd = "./weblogo/weblogo -F pdf -f  %s --errorbars NO -o %s" %('tmp',outname+'.pdf')
	run = Popen(cmd.split())
	run.communicate()
	os.remove('tmp')
	...
	for line in lines:
		...
		for i in frameList:
			if rf == i:
				...
				break
		if not found:
			print 'error in frames of peptidefile!\n'
			sys.exit(1)
	n = len(pep)
	for rf in d.keys():
		if len(d[rf])!= n:
			print 'all the peptides must have equal length!\n'
			sys.exit(1)
		for ch in d[rf]:
			if ch not in IupacAA:
				print 'error in peptide sequences!\n'
				sys.exit(1)
	for rf in frameList:
		if rf not in d.keys():
			d[rf] = 'O'*n
	...
	for i in range(len(args)):
		if args[i]=='-c':
			if i+1!=len(args) and args[i+1][0]!='-':
				...
				print "constraint file must be secified after -c flag!\n "
				printusage(sys.argv[0])
		else:
			constraintfile = None
		...
		if args[i]=='-n':
			if i+1!=len(args) and args[i+1][0]!='-':
				if args[i+1].isdigit():
					...
					print "error in -n flag!\n"
					printusage(sys.argv[0])
				del[args[i+1]]; del[args[i]]
				break
			else:
				print "number of samples must be specified after -n flag!\n"
				printusage(sys.argv[0])
	...
	for i in range(len(args)):
		if args[i]=='-m':
			if i+1!=len(args) and args[i+1][0]!='-':
				...
				print "matrix file must be specified after -m flag!\n"
				printusage(sys.argv[0])
		else:
			filename = None
	...
	for i in range(len(args)):
		if args[i]=='-t':
			if i+1!=len(args) and args[i+1][0]!='-':
				...
				print "similarity threshold must be specified after -t flag!\n"
				printusage(sys.argv[0])
		else:
			threshold = 1
	...
	for i in range(len(args)):
		if args[i]=='-gc':
			if i+1!=len(args) and args[i+1][0]!='-':
				...
				print "gc-content must be specified after -gc flag!\n"
				printusage(sys.argv[0])
	...
	for i in range(len(args)):
		if args[i]=='-pssm':
			if i+1!=len(args) and args[i+1][0]!='-':	
				...
				print "pssm file name must be specified after -pssm flag!\n"
				printusage(sys.argv[0])
